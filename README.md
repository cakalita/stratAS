...
* When --cell_specific flag is used: A cell fraction file with row per individual (needs to be labeled as sample_ID), columns per cell type. Also need a gene_express file containing gene expression with "gene name" and then columns for expression in each individual (labeled).
* When --pheno flag is used: A samples file with the columns `ID` and `CONDITION` that is in the **same order** as the vcf input. `CONDITION` is currently restricted to 0/1. This file got into the `--samples` flag.
...
* Global/local parameter files generated by `params_new.R` (see below). At minimum a global overdispersion parameter file with the columns "ID","CHR","P0","P1","PHI","MU","N" with entries in any order goes into the `--global_params` flag.
...
### `stratas_DeCAF.R` : testing for differential AS

`stratas_DeCAF.R` computes the AS statistics from a VCF of all individuals and the prior parameters estimated above.
